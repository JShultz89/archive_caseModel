...
def solve(heatGen,waterTemp,n):
	...
	for i in range(1,n+1):
		...
		# air.append(b.Block('airModule' + str(i/2),'air',T = 22.5))
		water[i].addSource(s.Source('const',T = -heatGen[i/2-1]*1e-3))
		air[i].addSource(Sa)
	...
	if len(sys.argv) < 4:
		...
		for row in cr:
			...
			cw.writerow({'Timestamp':row['Timestamp'],'exp_inlet':row['exp_inlet'], \
				'exp_outlet':row['exp_outlet'],'sim_outlet':round(Wt[-1],8)})
			TsimW.append(Wt[-1])
			AllW.append(Wt)
			# TsimA.append(Ta)
		...
		# plt.show()
		plt.savefig('feb11_oneblock.png')
		plt.close()
