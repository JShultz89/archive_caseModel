...
waterTemp = 20
w0 = b.Block('waterInlet','constWater',T = csvcol['exp_inlet'][start])
...
w0.addSource(s.Source('time', T = lambda t: interp1d(time,csvcol['exp_inlet'])(t),kind='linear' ))
a0.addSource(s.Source('time', T = lambda t: 22 ))


w0.mdot = lambda w0 : interp1d(time,csvcol['exp_flowrate'],kind='linear')(w0.t)*1.0e-6*w0.m['rho'](w0.state)
...
for i in range(1,2*n+1):
	...
	if(i % 2 == 1): # odd regions are "tube" regions
		...
		water.append(b.Block('waterTube' + str((i+1)/2),'constWater',T = csvcol['exp_outlet'][start]))
		...
		# Every block is named for its material in this case
		water.append(b.Block('waterModule' + str(n-i/2+1),'water',T =  csvcol['exp_outlet'][start]))
		air.append(b.Block('airModule' + str(n-i/2+1),'air',T = 22))
		water[i].addSource(s.Source('time', T = lambda t: -0.001*interp1d(time,csvcol['heatgen_m'+str(n-i/2+1)])(t),kind='linear'))
	...
	water[i].mdot = lambda w0 : interp1d(time,csvcol['exp_flowrate'],kind='linear')(w0.t)*1.0e-6*w0.m['rho'](w0.state)
...
ICSolar.solve(time[start]);
soln = ICSolar.solveUnst(time[start:end])
t0 = time[0];
# for i in range(1,7):
print [(s,soln[s][-1]) for s in soln]
plt.plot(np.array(soln['t'])-t0, np.array(soln['waterModule1_T']),linewidth=2.0,label='Model');
plt.plot(np.array(time[start:end])-t0, np.array(csvcol['exp_outlet'][start:end]),linewidth=2.0,label='Expt');
plt.plot(np.array(time[start:end])-t0, np.array(csvcol['exp_inlet'][start:end]),linewidth=2.0,label='Inlet');

plt.xlabel('Time (s)')
plt.ylabel('Temperature (C)')
plt.legend(loc=0)

plt.savefig('Feb6_unst_10.png')
# plt.show()
plt.close()